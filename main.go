...
func ProcessAirdrops() {
	...
	// 创建空投服务
	airdropService := NewAirdropService(cfg)
	...
	if msg != "" {
		// 读取上次的快照
		lastSnapshot, err := LoadLastSnapshot("last_snapshot.txt")
		if err != nil {
		...
		if !airdropService.CompareSnapshots(snapshot, lastSnapshot) {
			...
			if isOnlyDeletion {
				...
				// 保存当前快照但不推送
				if err := SaveSnapshot(snapshot, "last_snapshot.txt"); err != nil {
					fmt.Printf("保存快照失败: %v\n", err)
				}
			} else {
				...
				// 推送通知
				if err := SendToServerChan(msg, "今日空投播报"); err != nil {
		...
		if err == nil && lastSnapshot != "" {
			fmt.Println("清空快照文件...")
			if err := SaveSnapshot("", "last_snapshot.txt"); err != nil {
				fmt.Printf("清空快照失败: %v\n", err)
			}
		}
	}
